...
-------------------------------
SELECT * 
FROM EMPLOYEE; -- DEPT_CODE 값이 NULL인 행이 존재(2행)

SELECT * 			
FROM DEPARTMENT ; -- DEPT_ID 값이 NULL인 행이 없다(0행)
...
-- 21행 조회
-- 왜? 23행이 아니라 21행 
-- EMPLOYEE에 DEPT_CODE 값 중 2개의 NULL 값이 존재하지만
-- DEPATMENT에 DEPT_ID 값에는 NULL 존재 하지 않기때문 두개는 연결X
...
-- 2) USIN 이용 방법 -> JOIN에 사용된 컬럼이 1개로 합쳐짐
SELECT
 ...
 * -> 같은타입, 같은 이름의 컬럼이 없을 경우 발생
 * -> 결과 CROSS JOIN 형태(모든 경우의 수 )가 조회된다
 * */
...
SELECT 
	EMP_NAME, DEPT_TITLE ,LOCAL_NAME -- 23행 14열
	-- EMPLOYEE 컬럼 1개
	-- + DEPARTMENT 컬럼 3개(DEPT_ID,DEPT_TITLE,LOCATION_ID)
			--> DEPARTMENT.LOCATIONID 컬럼이
...
FROM
	EMPLOYEE E
JOIN
	JOB J ON (E.JOB_CODE = E.JOB_CODE)
WHERE 
	E.EMP_NAME LIKE '%형%';
...
SELECT 
	E.EMP_NAME 사원명,D.DEPT_TITLE 부서명,
	L.LOCAL_NAME 지역명,
	N.NATIONAL_NAME 국가명
FROM
	EMPLOYEE E
