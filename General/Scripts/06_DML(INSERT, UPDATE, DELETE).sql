...
900, '장채현', '001123-2345678', 'jang_ch@kh.or.kr', 
...
UPDATE EMPLOYEE2 SET 
	(SALARY,BONUS) = ( -- 수정하겠다 서브쿼리에 나오는 셀렉트로
		SELECT SALARY,BONUS 
		FROM EMPLOYEE2 
		WHERE EMP_NAME = '유재식'
	)
...
-- 컬럼 값이 같은 행이 있을경우 -> UPDATE
-- 컬럼 값이 같은 행이 없을경우 -> INSERT
...
/* INOT 빠져있음
 * 왜? MERGE 선언부에 "INTO EMP_M01이" 이미 작성되어서
 *  */
