...
/* [MEMBER 테이블 설계] 
 * 
 * - 영어,숫자(1BYTE), 한자,한글(3BYTE)
 * - 필요한 컬럼 : 아이디, 비밀번호, 이름, 주민등록번호, 가입일
 * 
 * - 컬럼명, 자료형 생각하기
 * 
 *             컬럼명          	     자료형(크기)
 * 아이디       : MEMBER_ID       	VARCHAR2(20[BYTE])
 * 비밀번호     : MEMBER_PW       	VARCHAR2(20)
 * 이름         : MEMBER_NAME       VARCHAR2(15) (한글 5글자까지) 3BYTEX5
 * 주민등록번호 : MEMBER_SSN    	  CHAR(14)
...
INSERT INTO "MEMBER" 
VALUES('MEM03','QWER1235','박철수','787878-3456789',DEFAULT);
...
VALUES('MEM04','QWER1236','신짱구','787879-3456781');
...
CREATE TABLE USER_USED_UK(
    ...
    -- 컬럼 레벨로 UNIQUE 설정
    --USER_ID VARCHAR2(20) CONSTRAINT USER_IDE_U UNIQUE, 
    USER_ID VARCHAR2(20),
...
--> FOREING KEY가 설정된 컬럼의 값은
--  외부 테이블의 값을 저장(사용)할 수 있다
...
체크 제약조건(KH25_JDK.CHECK_SCORE)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	...
	PNO CHAR (14) CONSTRAINT NN_PNO PRIMARY KEY,
	GENDER CHAR(3) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여')),
	PHONE CHAR(13),
	ADDRESS VARCHAR2(30),
	STATUS VARCHAR2(30) CONSTRAINT NN_NTAUS NOT NULL,
	
	--NOT NULL 은 컬럼레벨만 이용!!!!
	
	-- 테이블 레벨
	--CONSTRAINT PK_USER_TEST PRIMARY KEY(USER_NO),
	--CONSTRAINT UK_USER_ID PRIMARY KEY(USER_ID),
...
--> 서브쿼리로 테이블 생성 시
-- 별도의 제약조건 설정, DEFAULT 설정, COMMNET 설정 등이 필요!
...
-- FOREIGN KEY(컬럼명) REFERENCES 참조 테이블명(참조컬럼명)
ALTER TABLE EMPLOYEE --자식테이블
ADD CONSTRAINT DEPT_CODE_ID
FOREIGN KEY (DEPT_CODE) 
REFERENCES DEPARTMENT(DEPT_ID); -- 부모테이블
