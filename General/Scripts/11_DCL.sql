...
GRANT CREATE SESSION TO TEST25_USER;
--> 연결 확인됨

/* 생성된 사용자 계정으로 접속(TEST25_USER) */
CREATE TABLE TEST_TABLE(
	TEST_NO NUMBER PRIMARY KEY,
	TEST_COL VARCHAR2(30) NOT NULL
);
--> 권한이 불충분합니다
 -- + 사용 가능한 공간(TABLESAPCE) 할당

/* 다시 관리자로 접속 */
GRANT RESOURCE TO TEST25_USER;

/* ROLE(역할) : 권한의 묶음 
 *  - CONNECT : 접속 관련 권한 묶음
 *  - RESOURCE : 객체 생성 권한 묵음
 * */
--SELECT * FROM ROLE_SYS_PRIVS
--WHERE ROLE = 'RESOURCE';

/* TEST25_USER에게 테이블 생성 공간 할당
 * - USERS폴더 내에 10M(메가) 할당
 *  */
ALTER USER TEST25_USER
DEFAULT TABLESPACE USERS
QUOTA 10M ON USERS; -- 공용시스템계정



/* 다시 TEST25_USER 계정 접속후 테이블 생성 */
CREATE TABLE TEST_TABLE(
	TEST_NO NUMBER PRIMARY KEY,
	TEST_COL VARCHAR2(30) NOT NULL
);
