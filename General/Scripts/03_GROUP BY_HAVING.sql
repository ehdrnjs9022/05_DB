...
SELECT 
	DEPT_CODE,FLOOR(AVG(SALARY))  "평균 급여" -- (4) 그룹별 평균 조회
FROM
...
FROM
	EMPLOYEE 
WHERE 
	EXTRACT (YEAR FROM HIRE_DATE) > = 2010 -- 17행
	-- HIRE_DATE에서 년도만 뽑아내겠다 
GROUP BY
	JOB_CODE;
