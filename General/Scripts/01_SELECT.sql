...
/* * DB는 날짜 / 시간 관련 데이터를 다룰 수 있도록 하는 자로형 제공
 * 
 *  - DATE 타입 : 년,월,일,시,분,초,요일 저장
 * 	- TIMESTAMP 타입 : 년,월,일,시,분,초,요일, ms, 지역 저장
 * 
 * - SYS (시스템) : 시스템에 설정된 시간을 기반
 * - CURRENT : 현재 접속한 세션(사용자)의 시간을 기반
 * 
 *  - SYSDATE : 현재 시스템 시간 얻어오기
 *  - CURRENTDATE : 현재 사용자 계정 기반 시간 얻어오기
 * 
...
SELECT 
	...
	EMP_NAME ,
	HIRE_DATE ,
	FLOOR(CURRENT_DATE - HIRE_DATE ), -- 소수점 내림 처리
 ...
 * 		'AB', '가나다'('' 홑따옴표)
 *  
 *  * SELECT절(SELCET 한줄, FORM문까지는 SELECT문)에 리터럴을 작성하면
 ...
 * 
 * 2) 컬럼명 IS NOT NULL : 해당 컬럼 값이 NULL이아니면 TRUE 반환
 * 												== 컬럼에 값이 존재하면 TRUE
 * (컬럽 값의 존재 유무를 비교하는 연산)
 * */
 ...
 * 
 * - OR(또는)
 * 	두 연산자의 결과가 FLASE 일때만 최종 결과 FALSE
 * 	->두 조건을 중 하나라도 만족하는 행을
 * 		결과 집합(RESULT SET)에 포함
 * 
 * - 우선 순위 : AND > OR 
 * */
...
FROM
	EMPLOYEE 
WHERE 
	DEPT_CODE = 'D6' AND SALARY > 4000000;
...
-- 급여가 400미만, 600만 초과인 사원 조회
SELECT 
	EMP_NAME, SALARY
FROM
	EMPLOYEE
WHERE
	SALARY NOT BETWEEN 4000000 AND 6000000; -- 17행 조회
...
-- EMAIL 아이디 중 '_(언더스코어)' 앞 글자 수가 3글자인 사원의
-- 사번, 이름, 이메일
SELECT 
	EMP_ID, EMP_NAME, EMAIL
FROM
 ...
 * 
 * - 작성법
 * 	WHERE LIKE '___#_' ESCAPE '#'  -- !,@,# 특수문자 상관없음
 * 	-> '#' 바로 뒤 '_'는
 * 			와일드 카드 X, 단순 문자열 O
 * */
...
FROM
	EMPLOYEE 
WHERE 
	EMAIL LIKE '___#_%' ESCAPE '#'; -- 언더스코어 앞에 3글자
...
-- 연봉 오름 차순으로 정렬
SELECT 
	EMP_ID 사번 ,EMP_NAME 이름 ,SALARY * 12 연봉
FROM
	EMPLOYEE 
ORDER BY
--	SALARY * 12 ASC; 
...
WHERE 
	SALARY * 12 >= 50000000
	-- 연봉 *12 >= 50000000 오류발생 -> 아직 별칭 인식 전
ORDER BY 
	연봉 ASC;
...
-- 이름, 부서코드, 직급코드를 조회
SELECT 
	EMP_NAME 이름 ,DEPT_CODE "부서 코드" ,JOB_CODE "직급 코드"
FROM
	EMPLOYEE 
ORDER BY
	"부서 코드" ASC , "직급 코드" DESC, 이름 ASC;
