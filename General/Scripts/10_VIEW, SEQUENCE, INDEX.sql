...
--> DEPT_COPY2 테이블 제약조건 확인
-- DEPT_ID, LOCATION_ID : NOT TNULL
-- DEPT_TITLE : NULL
...
SELECT * FROM DEPT_COPY2 ; -- DO NULL L2
...
-- V_DCOPY2를 이용하면 LOCATION_ID 컬럼에 INSERT 할 수 없기떄문에
...
SELECT *FROM TB_TEST; -- 145, 150

--------------------------------

-- SEQUENCE 변경(ALTER)

-- CREATE와 SEQUENCE 작성법이 유사함
--> 단, START WITH 제외
...
-- 150 이상으로 증가 불가(MAXVALUE)
SELECT *FROM TB_TEST; -- 160,170 ~ 200
-----------------------------------------------------
 ...
 *  */
SELECT * FROM TB_IDX_TEST 
WHERE TEST_ID = 'TEST500000'; -- 0.04초

/* 인덱스를 이용해서 조회하기
 * - TEST_NO 컬럼 값이 'TEST500000'인 행 조회 
 *  */
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; -- 0.03초
