...
-- WHERE :
-- FROM절 테이블에서 조회를 원하는 행에 조건
-- HAVING
-- GROUP BY절에 의해 만들어진 그룹중 조회를 원하는 그룹에 대한 조건
SELECT 
...
HAVING 
	SUM(SALARY) =(
	SELECT MAX(SUM(SALARY)) -- 서브쿼리 -> 21700000
	FROM EMPLOYEE E
	GROUP BY DEPT_CODE 
);
...
-- 3) >> 위의 두 서브쿼리 합쳐 부서별 급여 합이 큰 부서의 부서명,
-- 			급여 합 조회
...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한
--     DEPT_CODE를 가지는 사원을 조회
...
SELECT 
	E.EMP_ID ,E.EMP_NAME ,D.DEPT_ID ,E.DEPT_CODE ,J.JOB_NAME 
FROM
	EMPLOYEE E
JOIN 
	DEPARTMENT D ON (E.DEPT_CODE=D.DEPT_ID)
...
/* EXISTS(서브쿼리)
 * - 서브쿼리 조회 결과가 
 * 	 1행 이상 존재하면 TURE 				-> MAIN QUERY 결과 포함
 * 	 조회 결과가 0행이면 FALSE 반환 -> MAIN QUERY 결과 미포함
 * 
 *  */
 ...
 * 
 * 3) (메인 쿼리 WHERE 절 사용 시 )
 * 		서브쿼리의 결과가
 * 		먼저 해석된 메인 쿼리 1행에 영향을 줌
 * 		-> 최정 결과(RESULT SET)에 포함 될지 안될지를 지정
 * 
 *  */
...
SELECT 
	MAIN.EMP_ID ,MAIN.EMP_NAME ,NVL(D.DEPT_TITLE,'없음'),
...
WHERE 
	...
	-- ORWNUM BETWEEN 3 AND 7;

	-- "순위" 이용 -> 실존 컬럼 이용시 문제 없음!
	"순위" BETWEEN 3 AND 7;
	----------------------------------------------------------------------
/* 실습 문제 */
...
-- 사번, 사원명, 전화번호, 고용일, 부서명
SELECT 
	MAIN.EMP_ID,MAIN.EMP_NAME,MAIN.PHONE,
	MAIN.HIRE_DATE,SUB.DEPT_TITLE 
FROM
	EMPLOYEE MAIN
JOIN 
	DEPARTMENT SUB ON (MAIN.DEPT_CODE=SUB.DEPT_ID)
WHERE 
	SUB.DEPT_TITLE = (
		SELECT DEPT_TITLE 
		FROM DEPARTMENT 
...
SELECT 
	...
	E.JOB_CODE ,D.DEPT_TITLE ,J.JOB_NAME 
FROM
	EMPLOYEE E
JOIN
	DEPARTMENT D ON (E.DEPT_CODE=D.DEPT_ID)
...
-- 5. 87년생 여자 사원과 동일한 부서이면서 동일한 사수를 가지고 있는 
-- 사원을 조회하시오
-- 사번, 이름, 부서코드, 사수번호, 주민번호, 고용일
...
-- 단, 퇴사한 직원은 제외하고 조회..
SELECT 
	E.EMP_ID ,E.EMP_NAME ,NVL(DEPT_TITLE ,'소속없음') ,
...
WHERE 
	HIRE_DATE = (
		...
		FROM EMPLOYEE SUB
		WHERE ENT_YN != 'Y'
		AND NVL( SUB.DEPT_CODE,'소속없음') = NVL( E.DEPT_CODE,'소속없음') 
	)
ORDER BY 
	E.HIRE_DATE ASC;
...
SELECT 
	E.EMP_ID,E.EMP_NAME,J.JOB_NAME ,
	FLOOR( 
		MONTHS_BETWEEN(SYSDATE, TO_DATE(19 || SUBSTR(EMP_NO,1,6),'YYMMDD') )/12 
		)AS "만 나이",
...
WHERE 
	TO_NUMBER( SUBSTR(E.EMP_NO,1,6) ) = (
		...
		FROM EMPLOYEE  SUB
		WHERE 
			SUB.JOB_CODE = E.JOB_CODE  --같은 부서 안에서 제일어린사람 조회
	)
ORDER BY
	"만 나이" DESC;
